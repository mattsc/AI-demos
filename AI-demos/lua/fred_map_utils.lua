    ...
    for typ,id in pairs(overall_new_types) do
        ...
        for zone_id,cfg in pairs(zone_cfgs) do
            ...
            for x,y,data in FGM.iter(tmp_zone_maps[zone_id][typ]) do
                ...
                local old_cost_to_enemy_leader = FGM.get_value(combined_maps[typ], x, y, 'cost_to_enemy_leader')
                if (not old_cost_to_enemy_leader) or (new_cost_to_enemy_leader < old_cost_to_enemy_leader) then
                    FGM.set_value(combined_maps[typ], x, y, 'cost_to_enemy_leader', new_cost_to_enemy_leader)
                end
            end
        end
                ...
                for x,y in FGM.iter(new_hexes_map) do
                    ...
                    for xa,ya in H.adjacent_tiles(x, y) do
                        ...
                        if forward then
                            ...
                            if (new_cost_to_enemy_leader < min_new_cost_to_enemy_leader) then
                                min_new_cost_to_enemy_leader = new_cost_to_enemy_leader
                            end
                        end
                    end
...
function fred_map_utils.get_influence_maps(fred_data)
    ...
    local influence_map, influence_maps = {}, {}
        ...
        for x,y,data in FGM.iter(move_data.my_attack_map[int_turns]) do
            for _,id in pairs(data.ids) do
                ...
                if (int_turns == 1) then
                    ...
                    if (not influence_maps[id]) then influence_maps[id] = {} end
                    FGM.add(influence_maps[id], x, y, 'influence', my_influence)
                end
        ...
        for x,y,data in FGM.iter(move_data.enemy_attack_map[int_turns]) do
            for _,enemy_id in pairs(data.ids) do
                ...
                if (int_turns == 1) then
                    ...
                    if (not influence_maps[enemy_id]) then influence_maps[enemy_id] = {} end
                    FGM.add(influence_maps[enemy_id], x, y, 'influence', enemy_influence)
                end
    ...
    if DBG.show_debug('ops_influence_map') then
