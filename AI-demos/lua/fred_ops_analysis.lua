        ...
        --then
            ...
            for id,_ in pairs(zone_units) do
                ...
                if (not skip_unit) then
                    local unit_value = fred_data.move_data.unit_infos[id].unit_value
...
function fred_ops_analysis.set_ops_data(fred_data)
    ...
    if objectives.leader.village then
        ...
        leader_heal_benefit = 1.5 * leader_heal_benefit / move_data.unit_infos[leader.id].max_hitpoints * move_data.unit_infos[leader.id].unit_value
...
show_timing_info(fred_data,'A')
    ...
    --  -  'cost_to_my_leader' and 'cost_to_enemy_leader' are not set
    ...
    if DBG.show_debug('ops_advance_distance_maps') then
        for zone_id,uadm in pairs(advance_distance_maps) do
            ...
            DBG.show_fgm_with_message(advance_distance_maps[zone_id][typ], 'cost_to_my_leader', 'advance_distance_maps[' .. zone_id .. '][' .. typ .. ']: cost_to_my_leader')
    ...
    for id,_ in pairs(unused_units) do
        if (utilities.retreat[id] > 0) then
            ...
            heal_benefit = heal_benefit / move_data.unit_infos[id].max_hitpoints * move_data.unit_infos[id].unit_value
            local action = {
                id = id,
...
tmp_unit = nil
    ...
    for zone_id,ADmap in pairs(zone_advance_distance_maps) do
        ...
        for x,y,_ in FGM.iter(zone_maps[zone_id]) do
            ...
            if data then
                ...
                if (data.perp < 1.1) and (enemy_infl < (line_enemy_infl[int_forward] or math.huge)) then
                    ...
                    if (enemy_infl < min_enemy_infl) then
                        min_enemy_infl = enemy_infl
                    end
                end
            end
        end
        ...
        if objectives.protect.zones[zone_id] and objectives.protect.zones[zone_id].holders and next(objectives.protect.zones[zone_id].holders) then
            ...
            for forward,data in pairs(line_enemy_infl) do
                if (data < min_enemy_infl + 0.5) and (forward > max_forward) then
...
function fred_ops_analysis.get_action_cfgs(fred_data)
    ...
    local base_ratings = FCFG.get_cfg_parm('action_base_ratings')
        ...
        if (zone_id ~= 'leader') then
            ...
            if holders_by_zone[zone_id] then
                ...
                if protect_obj and (
                    ...
                    if (base_ratings.protect > 0) then
                        rating = power_rating + base_ratings.protect
                        if (base_ratings.hold > 0) then
                            second_rating = power_rating + base_ratings.hold
                        end
                        action_str = 'zone protect'
                    end
                else
                    if (base_ratings.hold > 0) and (not protect_obj.already_holding) then
