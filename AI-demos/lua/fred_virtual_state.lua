...
local function adjust_move_data_tables(old_locs, new_locs, store_units_in_way, move_data)
    ...
    if (not store_units_in_way) then
        for id,loc in pairs(FVS_stored_units) do
...
function fred_virtual_state.set_virtual_state(old_locs, new_locs, additional_units, move_data)
    FVS_stored_units, FVS_ids, FVS_add_units = {}, {}, {}

    -- Mark the new positions of the units in the move_data tables
    adjust_move_data_tables(old_locs, new_locs, true, move_data)
...
function fred_virtual_state.reset_state(old_locs, new_locs, move_data)
    -- Extract the units from the map
    for _,add_unit in ipairs(FVS_add_units) do
    ...
    adjust_move_data_tables(new_locs, old_locs, false, move_data)
